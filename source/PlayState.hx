...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'bus':
				var sky:BGSprite = new BGSprite('skything', 0, 0, 0.1, 0.1);
				sky.screenCenter();
				add(sky);
				...
				var bus:BGSprite = new BGSprite('cima del bus turistica', 0, 0, 1, 1);
				bus.screenCenter();
				bus.y += 100;
				bus.x += 100;
				add(bus);

				rat = new BGSprite('rat', 0, 0, 1, 1, ['full idle instance 1'], false);
				rat.screenCenter();
				rat.y -= 100;
				rat.x += 200;
				add(rat);

				bgppl = new BGSprite('bg chairs', 0, 0, 1, 1, ['bg chairs idle'], false);
				bgppl.setGraphicSize(Std.int(bgppl.width * 2));				
				bgppl.screenCenter();
				bgppl.y += 150;
				bgppl.x += 150;
				add(bgppl);

				fgppl = new BGSprite('fg chairs', 0, 0, 1, 1, ['fw chair idle'], false);
				fgppl.screenCenter();
				fgppl.scale.set(2, 2);
				fgppl.y += 400;
				fgppl.x += 150;
			...
			case 'bus_afternoon':
				var sky1:BGSprite = new BGSprite('skything_afternoon', 0, 0, 0.1, 0.1);
				sky1.screenCenter();
				add(sky1);
				...
				var bus1:BGSprite = new BGSprite('cima del bus turistica afternoon', 0, 0, 1, 1);
				bus1.screenCenter();
				bus1.y += 100;
				bus1.x += 100;
				add(bus1);

				rat1 = new BGSprite('rat_afternoon', 0, 0, 1, 1, ['full idle instance 1'], false);
				rat1.screenCenter();
				rat1.y -= 100;
				rat1.x += 200;
				add(rat1);

				bgppl1 = new BGSprite('bg chairs afternoon', 0, 0, 1, 1, ['bg chairs idle'], false);
				bgppl1.screenCenter();
				bgppl1.scale.set(2, 2);
				bgppl1.y += 150;
				bgppl1.x += 150;
				add(bgppl1);

				fgppl1 = new BGSprite('fg chairs afternoon', 0, 0, 1, 1, ['fw chair idle'], false);
				fgppl1.screenCenter();
				fgppl1.scale.set(2, 2);
				fgppl1.y += 400;
				fgppl1.x += 150;
		}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (!skipArrowStartTween)
			{
				babyArrow.y -= 40;
				babyArrow.alpha = 0;
				FlxTween.tween(babyArrow, {y: babyArrow.y + 40 , alpha: targetAlpha}, 1, {ease: FlxEase.circOut, startDelay: Conductor.crochet / 1000 + (Conductor.crochet / 1000 * i)});
			}
			else
			{
	...
	override public function update(elapsed:Float)
	{
		...
		switch (ClientPrefs.iconBop)
		{
			case 'Modern':
				iconP1.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - 25;
				iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - 125;
	...
	public function moveCamera(isDad:Bool)
	{
		...
		else
		{
			...
			{
				cameraTwn = FlxTween.tween(FlxG.camera, {zoom: 1}, (Conductor.stepCrochet * 4 / 500), {ease: FlxEase.quintOut, onComplete:
					function (twn:FlxTween)
					{
						cameraTwn = null;
					}
	...
	function tweenCamIn() {
		if (Paths.formatToSongPath(SONG.song) == 'tutorial' && cameraTwn == null && FlxG.camera.zoom != 1.3) {
			cameraTwn = FlxTween.tween(FlxG.camera, {zoom: 1.3}, (Conductor.stepCrochet * 4 / 1000), {ease: FlxEase.quintOut, onComplete:
				function (twn:FlxTween) {
					cameraTwn = null;
				}
			});
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (curStage == 'bus' || curStage == 'bus_afternoon')
			{
				rating.velocity.x += FlxG.random.int(400, 450);
			}
			else
			{
		...
		if (curStage == 'bus' || curStage == 'bus_afternoon')
		{
			comboSpr.velocity.x += FlxG.random.int(400, 450);
		}
		else
		{
		...
		for (i in seperatedScore)
		{
			...
			if (curStage == 'bus' || curStage == 'bus_afternoon')
				{
					numScore.velocity.x += FlxG.random.int(400, 450);
				}
				else
				{
